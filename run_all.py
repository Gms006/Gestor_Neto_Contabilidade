#!/usr/bin/env python3
"""
Orquestrador único para executar o pipeline completo.
Detecta primeira execução e usa full=True automaticamente.
...
def is_first_execution(root):
    """Detecta se é a primeira execução verificando se o banco de dados existe."""
    db_path = root / "data" / "gestor.db"
    snapshot_path = root / "data" / "meta.json"
    
    # Se nenhum dos dois existe, é primeira execução
    return not (db_path.exists() or snapshot_path.exists())
...
def main():
    ...
    first_execution = is_first_execution(root)
    
    print_header("GESTOR NETO CONTABILIDADE - PIPELINE")
    print(f"Raiz do projeto: {root}\n")
    
    if first_execution:
        print_warning("Primeira execução detectada. Usando full=True para coleta completa.\n")
    else:
        print(f"Execução incremental (use --full para forçar coleta completa).\n")
    
    # Pipeline de 7 etapas
    steps = [
        ("scripts.fetch_api", "Coletando processos da API", ["--full"] if first_execution else []),
    ...
    for i, (module, description, extra_args) in enumerate(steps, 1):
        print_step(i, len(steps), description)
        
        try:
            # Executar módulo Python usando sys.executable (garante .venv)
