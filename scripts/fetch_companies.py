...
try:
    from scripts.db import get_session, init_db, bulk_upsert_companies
    DB_AVAILABLE = True
            ...
            for company in companies:
                ...
                if cnpj:
                    companies_data.append({
                        "cnpj": cnpj,
                        "nome": company.get("RazaoSocial") or company.get("Nome") or company.get("Fantasia") or ""
                    })
            
            count = bulk_upsert_companies(session, companies_data)
            session.close()
            log("fetch_companies", "INFO", "Persistido no banco de dados", total=count)
        except Exception as e:
            log("fetch_companies", "ERROR", "Erro ao persistir no banco", error=str(e))
